...
print x
points_first, x_first = g_first.gradient_descent(x, 80, BacktrackingLineStep(0.3,0.5,10))
points_first_mom, x_first_mom = g_first_mom.gradient_descent(x, 80, FixedStep(0.1))
points_second, x_second = g_second.gradient_descent(x, 80, BacktrackingLineStep(0.3,0.5,10))
points_second_mom, x_second_mom = g_second_mom.gradient_descent(x, 80, FixedStep(0.1))
points_bfgs, x_bfgs = g_bfgs.gradient_descent(x, 30, BacktrackingLineStep(0.3,0.5,10))
points_bfgs_mom, x_bfgs_mom = g_bfgs_mom.gradient_descent(x, 80, FixedStep(0.01))
...
plot(q, [-4.5, 4.5], [-4.5, 4.5], [(points_first, "First order"), (points_first_mom, "First order with momentum"), (points_bfgs, "BFGS without momentum"), (points_bfgs_mom, "BFGS with momentum"),(points_second, "Second order"), (points_second_mom, "Second order with momentum")])#, (points_cr, "Cubic Regularization") ])
